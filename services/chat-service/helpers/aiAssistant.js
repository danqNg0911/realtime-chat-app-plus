...
const getAssistantConfig = () => ({
  ...
  image:
    process.env.AI_ASSISTANT_IMAGE ||
    "https://i.pinimg.com/originals/02/ac/32/02ac324ac4256566ff7d2aad37defd15.jpg",
...
export const ensureAiAssistantUser = async () => {
  const { email, firstName, lastName, image, color, password } =
    getAssistantConfig();
  ...
  if (!assistant) {
    ...
    if (assistant.firstName !== firstName) {
      assistant.firstName = firstName;
      updated = true;
    }
    if (assistant.lastName !== lastName) {
      assistant.lastName = lastName;
      updated = true;
    }
    if (assistant.image !== image) {
      assistant.image = image;
      updated = true;
    }
    if (assistant.color !== color) {
    ...
    if (updated) {
      ...
      console.log("✅ AI Assistant profile updated from .env");
    }
  }
...
const mapConversationToGemini = (conversation, assistantId) => {
  return conversation
    ...
    .map((message) => ({
      role:
        message.sender.toString() === assistantId.toString() ? "model" : "user",
      parts: [{ text: message.content }],
    }));
};

export const generateAssistantReply = async (userId, assistantId) => {
  const { prompt, model, temperature, maxTokens, historyLimit } =
    getAssistantConfig();
...
export const getAssistantPublicProfile = async () => {
  const assistant = cachedAssistant || (await ensureAiAssistantUser());
  return {
    _id: assistant._id,
