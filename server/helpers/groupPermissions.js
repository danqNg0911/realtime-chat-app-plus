...
export const detectBlockingConflictForMembers = async (memberIds = [], requesterId) => {
    ...
    for (let i = 0; i < uniqueIds.length; i += 1) {
        for (let j = i + 1; j < uniqueIds.length; j += 1) {
            ...
            if (blockResult.blocked) {
                // Determine appropriate message based on who the requester is
                let message;
                if (blockResult.blocker === requesterIdStr) {
                    ...
                    // Requester is trying to add two people who have blocked each other
                    message = "These users have blocked each other and cannot be in the same group.";
                }
...
export const detectBlockingConflictBetweenSets = async (
    ...
    for (const candidateId of uniqueCandidateIds) {
        for (const existingId of uniqueExistingIds) {
            ...
            const blockResult = hasBlockingConflict(blockedMap, candidateId, existingId);
            ...
            if (blockResult.blocked) {
                ...
                if (blockResult.blocker === requesterIdStr) {
                    ...
                    // Requester is trying to add someone who has blocking conflict with existing member
                    message = "This user has a blocking conflict with an existing group member.";
                }
