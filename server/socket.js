  ...
  io.on("connection", (socket) => {
    ...
    socket.on("call:offer", async ({ to, callId, callType }) => {
      try {
        ...
        // Check block status before allowing call
        ...
        // Block call if either user blocked the other
        const callerBlockedRecipient = caller.blockedUsers?.some(id => id.toString() === to.toString());
        const recipientBlockedCaller = recipient.blockedUsers?.some(id => id.toString() === userId.toString());
