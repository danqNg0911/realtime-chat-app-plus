...
import { HOST, UPLOADS_HOST, UPDATE_PROFILE_ROUTE } from "../../utils/constants";
...
const resolveProfileImageUrl = (rawImage) => {
  ...
  if (/^uploads\//i.test(withoutLeadingSlash)) {
    const prefixedPath = normalized.startsWith("/")
      ? normalized
      : `/${normalized}`;
    return `${UPLOADS_HOST}${prefixedPath}`;
  }

  return `${HOST}/${withoutLeadingSlash}`;
};
...
const ProfileLandingPage = () => {
  ...
  useEffect(() => {
    ...
    if (userInfo.image) {
      setImage(resolveProfileImageUrl(userInfo.image));
    }
  }, [userInfo]);
    ...
    if (validateProfile()) {
      try {
        ...
        if (response.status === 200 && response.data) {
          ...
          // if (newPassword) toast.success("Password changed successfully");
          navigate("/chat");
        }
      } catch (error) {
        console.log(error);
