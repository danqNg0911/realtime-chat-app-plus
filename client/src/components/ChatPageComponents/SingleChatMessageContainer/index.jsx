...
const SingleChatMessageContainer = () => {
  ...
  // Helper function to get latest member info (including avatar)
  const getMemberInfo = (senderId) => {
    if (selectedChatType === "group" && selectedChatMembers.length > 0) {
      const member = selectedChatMembers.find((m) => m._id === senderId);
      if (member) return member;
    }
    return null;
  };

  // Handle click on member avatar/name to view profile
  const handleMemberClick = (senderId) => {
    const memberInfo = getMemberInfo(senderId);
    if (memberInfo) {
      setContactOrGroupProfile(memberInfo);
  ...
  const renderGroupMessages = (message) => {
    // Get latest member info from selectedChatMembers
    const memberInfo = getMemberInfo(message.sender._id);
    const senderImage = memberInfo?.image || message.sender.image;
    const senderFirstName = memberInfo?.firstName || message.sender.firstName;
    const senderLastName = memberInfo?.lastName || message.sender.lastName;
      ...
      >
        {message.sender._id === userInfo.id ? null : (
          <div
            ...
            onClick={() => handleMemberClick(message.sender._id)}
          ...
          {message.sender._id !== userInfo.id && (
            <div className="group-message-contact-info-above-content">
              <div
                ...
                style={{ color: "#53a6fd", cursor: "pointer" }}
                onClick={() => handleMemberClick(message.sender._id)}
              >{`${senderFirstName} ${senderLastName}`}</div>
            </div>
          )}
          {message.messageType === "text" && message.content}
          {message.messageType === "file" && message.fileUrl && (
