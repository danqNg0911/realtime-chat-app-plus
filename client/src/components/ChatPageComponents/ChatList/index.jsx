...
import {
  ...
  GET_AI_ASSISTANT_PROFILE_ROUTE,
...
const ChatList = () => {
  ...
  const [dmContactsFromServer, setDmContactsFromServer] = useState([]);
  const [assistantContact, setAssistantContact] = useState(null);
  ...
  useEffect(() => {
    const fetchAssistantContact = async () => {
      try {
        ...
        if (response.data.assistant) {
          setAssistantContact(response.data.assistant);
        }
      } catch (error) {
        console.log(error);
      }
    ...
    const getContacts = async () => {
      ...
      if (response.data.contacts) {
        setDmContactsFromServer(response.data.contacts);
      }
    };
    ...
    const getGroups = async () => {
      ...
      if (response.data.groups) {
        console.log("📥 Fetched groups from server:", response.data.groups);
        setGroups(response.data.groups);
        // addGroupInGroupList(response.data.groups);
      }
    };

    fetchAssistantContact();
    getContacts();
    getGroups();
  }, [refreshChatList, setGroups]);

  useEffect(() => {
    const mergeContacts = () => {
      let mergedContacts = [...dmContactsFromServer];

      if (assistantContact) {
        const assistantExists = mergedContacts.some(
          (contact) => contact._id === assistantContact._id
        );
          ...
          mergedContacts = [
            {
              ...assistantContact,
              lastMessage:
                assistantContact.lastMessage ||
                "Xin chào! Tôi là trợ lý AI của bạn.",
              lastMessageType:
                assistantContact.lastMessageType || "text",
              lastMessageTime:
                assistantContact.lastMessageTime || new Date().toISOString(),
            },
            ...mergedContacts,
          ];
        } else {
          mergedContacts = mergedContacts.map((contact) =>
            contact._id === assistantContact._id
              ? {
                ...contact,
                ...assistantContact,
                lastMessage:
                  assistantContact.lastMessage || contact.lastMessage,
                lastMessageType:
                  assistantContact.lastMessageType ||
                  contact.lastMessageType ||
                  "text",
                lastMessageTime:
                  assistantContact.lastMessageTime ||
      ...
      setDirectMessagesContacts(mergedContacts);
    };

    mergeContacts();
  }, [
    assistantContact,
    dmContactsFromServer,
    setDirectMessagesContacts,
  ]);
  ...
  const selectOptionTextColor = theme === "dark" ? "#111b21" : "#1f2933";
  const selectMultiValueColor = theme === "dark" ? "#111b21" : "#1f2933";
  const selectMultiValueBackground =
    theme === "dark" ? "lightgray" : "#e0e7ff";
                        ...
                        <div className="multi-select-container">
                          ...
                          <Select
                            ...
                            styles={{
                              ...
                              option: (styles) => ({
                                ...styles,
                                color: selectOptionTextColor,
                              }),
                              multiValue: (styles) => ({
                                ...styles,
                                backgroundColor: selectMultiValueBackground,
                              }),
                              multiValueLabel: (styles) => ({
                                ...styles,
                                color: selectMultiValueColor,
                              }),
