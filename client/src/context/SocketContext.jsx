...
import { SOCKET_HOST, SOCKET_PATH } from "../utils/constants";
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      const handleNewPhotoUploaded = (data) => {
        if (!data) {
          console.warn("⚠️ newPhotoUploaded payload missing");
          return;
        }
      ...
      const handlePhotoLiked = (data) => {
        if (!data?.photoId) {
          console.warn("⚠️ photoLiked payload missing photoId", data);
          return;
        }
      ...
      const handlePhotoUnliked = (data) => {
        if (!data?.photoId) {
          console.warn("⚠️ photoUnliked payload missing photoId", data);
          return;
        }
      ...
      const handlePhotoDeleted = (data) => {
        if (!data?.photoId) {
          console.warn("⚠️ photoDeleted payload missing photoId", data);
          return;
        }
      ...
      const handleCallBlocked = (data) => {
        const { resetCall } = useAppStore.getState();
        resetCall();
        toast.error(data.message || "Cannot call this user");
      };
      ...
      socket.current.on("call:blocked", handleCallBlocked);
      ...
      return () => {
        ...
        try { socket.current.off("call:blocked", handleCallBlocked); } catch (_) { }
